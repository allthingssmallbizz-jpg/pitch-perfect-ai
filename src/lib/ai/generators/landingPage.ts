...
1. **Hero — two columns, form always top-right.** A two-column layout (grid or flex, roughly 3:2 width split): the LEFT column holds the Big Promise headline (outcome-led, specific — if a webinar/VSL/sales page already exists above for this project, use its exact headline rather than a different one), a subheadline (who it's for + what they'll walk away with), and 2-3 short supporting trust bullets. The RIGHT column, positioned at the top of the page next to the hero copy (not below it, not further down the page), holds the opt-in form as a raised card: **Name** and **Phone Number** fields (type="tel" for phone — no email field), a submit button labeled with the actual CTA text, and one small reassurance line under the button (e.g. "We'll text your access details — no spam"). On narrow/mobile screens, stack the form directly under the hero copy so it still appears immediately, not lower on the page. No <script> tags and no form action/JS wiring — this is a static template the member connects to their own SMS/CRM system afterward; leave a single HTML comment noting that near the <form> tag.
...
- Establish a real visual identity: pick one accent color (a confident, modern color that fits the niche — not always the same blue) and use it consistently for CTAs, links, and small accents. Use CSS gradients, subtle shadows on cards/buttons, rounded corners, and generous whitespace to create genuine visual polish — this should look like a page built by a real designer, not a text document with a border around it.
