...
// Compares a member's own social page against a high-performing page they admire, using
// whatever Open Graph title/description/preview-image each platform actually exposes (see
// src/lib/social.ts for why that's the realistic ceiling on what can be read from TikTok/
// Instagram/Facebook without official API access). Deliberately framed around honesty about
// that limitation rather than pretending to have reviewed a full post history it was never
// actually shown.
...
function describeExtraction(label: string, p: SocialProfileExtraction): string {
  const lines = [
    ...
    p.bodyTextSnippet.length > 50
      ...
      : `Additional visible page text: (none available — this platform likely blocked automated reading beyond the basics above)`,
    p.image ? `A preview/thumbnail image for this page IS included below.` : `No preview image was available for this page.`,
  ];
  return lines.join("\n");
}
...
export function buildSocialComparePrompt(
  ...
  if (yours.image) {
    images.push(yours.image);
    imageLabels.push(`IMAGE ${images.length} = the preview image for YOUR page.`);
  }
  if (reference.image) {
    images.push(reference.image);
    imageLabels.push(`IMAGE ${images.length} = the preview image for the REFERENCE (high-performing) page.`);
  }
...
4. Where images are provided, describe what you actually see (composition, color, text overlay style, subject framing) rather than generic thumbnail advice.
...
## Visual Style${images.length ? "" : " (limited — no preview images were available to compare)"}
${images.length ? "Compare what's actually visible in the provided image(s) — composition, text overlays, color, subject framing." : "Note that no images were available and keep this section brief rather than inventing visual detail."}
...
## Data Limitations
One or two honest sentences on what wasn't available for either page and therefore isn't reflected above (e.g. "no visible post captions were readable for [page] — likely blocked by the platform").
...
${imageLabels.length ? imageLabels.join("\n") + "\n" : "No preview images were available for either page.\n"}
